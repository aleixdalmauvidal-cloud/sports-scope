...
export function ComparePageClient({ initialPlayers, preselectId }: Props) {
  ...
  const [selectedPlayers, setSelectedPlayers] = useState<Player[]>(() => {
    if (preselectId) {
      const p = initialPlayers.find((x) => x.id === preselectId);
      return p ? [p] : [];
    }
    return [];
  });
  ...
  }, [initialPlayers, searchQuery, selectedPlayers]);

  const addPlayer = useCallback((player: Player) => {
    setSelectedPlayers((prev) => {
      if (prev.length >= 4 || prev.some((p) => p.id === player.id)) return prev;
      return [...prev, player];
    });
    setSearchQuery("");
  ...
  const cmvMax = maxIndexSet(selectedPlayers.map((p) => p.cmvScore));
  const oppMax = maxIndexSet(selectedPlayers.map((p) => p.opportunityScore));
  const numericHighlights = SUB_ROWS.map((row) => maxIndexSet(selectedPlayers.map((p) => p[row.key] as number)));
  const scoreWins = selectedPlayers.map((_, idx) => {
    let wins = 0;
  ...
  const verdictText =
    bestIndexes.length === 1 && selectedPlayers[bestIndexes[0]!]
      ? `${selectedPlayers[bestIndexes[0]!]!.name} leads the comparison by winning ${bestScore} categories and is the strongest current brand-fit recommendation.`
      ...
      <main className="ml-20 min-h-screen">
        <div className="mx-auto max-w-[1600px] p-6 lg:p-8">
          <header className="mb-8">
            ...
            <p className="mt-1 text-sm text-muted-foreground">Compare CMV profiles side by side</p>
          </header>
          ...
          >
            ...
            <div className="flex flex-col gap-3">
              {selectedPlayers.length > 0 ? (
                <div className="flex flex-wrap items-center gap-2">
                  {selectedPlayers.map((p) => (
                    <span
                      key={p.id}
                      className="inline-flex items-center gap-2 rounded-lg border border-[rgba(56,160,71,0.22)] bg-card px-3 py-2 text-sm"
                    >
                      <span className="font-medium text-foreground">{p.name}</span>
                      <span className="font-display text-xs font-semibold text-[#38A047]">{formatScore(p.cmvScore)}</span>
                      <button
                        type="button"
                        aria-label={`Remove ${p.name}`}
                        className="rounded p-0.5 text-muted-foreground hover:bg-[rgba(45,122,58,0.08)] hover:text-foreground"
              ...
              <div
                className="relative min-w-[260px] max-w-xl"
                ref={searchWrapRef}
              ...
              >
                ...
                {showDropdown && dropdownMatches.length > 0 ? (
                  <ul
                    className="absolute left-0 right-0 top-full z-50 mt-1 max-h-[240px] overflow-y-auto rounded-lg border py-0 shadow-lg"
                    style={{
                      backgroundColor: "#1C2420",
                      borderColor: "rgba(56,160,71,0.2)",
                      borderRadius: "8px",
                    }}
                    role="listbox"
                  >
                    {dropdownMatches.map((p) => (
                      <li key={p.id}>
                        <button
                          type="button"
                          role="option"
                          className="flex w-full cursor-pointer items-center justify-between gap-3 px-[14px] py-[10px] text-left transition-colors hover:bg-[rgba(45,122,58,0.08)]"
                          onMouseDown={(e) => e.preventDefault()}
                          onClick={() => addPlayer(p)}
                        >
                          <span className="flex min-w-0 flex-1 items-center gap-3">
                            <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-[rgba(56,160,71,0.22)] bg-[rgba(45,122,58,0.12)] text-[10px] font-bold text-[#E8F5EA]">
                          ...
                          <span className="shrink-0 font-display text-sm font-bold text-[#38A047]">
                ...
                {showDropdown && dropdownMatches.length === 0 ? (
                  <p
                    className="absolute left-0 right-0 top-full z-50 mt-1 rounded-lg border px-[14px] py-[10px] text-sm text-muted-foreground shadow-lg"
                    style={{
                      backgroundColor: "#1C2420",
                      borderColor: "rgba(56,160,71,0.2)",
                      borderRadius: "8px",
            ...
            {selectedPlayers.length >= 4 ? (
              <p className="mt-2 text-xs text-muted-foreground">Maximum 4 players selected.</p>
            ) : null}
          </section>

          {selectedPlayers.length < 2 ? (
            <div
              className="rounded-[10px] border border-dashed border-border py-16 text-center text-sm text-muted-foreground"
              style={{ backgroundColor: CARD }}
            >
            ...
            <div className="overflow-x-auto rounded-[10px] border border-border" style={{ backgroundColor: CARD }}>
              <div
                ...
                style={{
                  gridTemplateColumns: `180px repeat(${selectedPlayers.length}, minmax(170px, 1fr))`,
                }}
              >
                ...
                {selectedPlayers.map((p) => (
                  ...
                  >
                    ...
                    <div
                      className={`flex min-h-[90px] flex-col items-center justify-center gap-2 rounded-lg border p-2 text-center ${
                        bestIndexes.includes(selectedPlayers.findIndex((s) => s.id === p.id))
                          ? "border-[rgba(56,160,71,0.22)]"
                          : "border-border"
                      }`}
                    >
                      <div
                ...
                <div className="section-title flex items-end border-b border-border px-3 py-3">Core Scores</div>
                <div className="border-b border-l border-border px-3 py-2.5 text-center text-xs text-[#7A9490]">
                  CMV + OPP + Breakdown
                </div>
                {selectedPlayers.slice(1).map((_, i) => (
                  <div
                    key={`core-${i}`}
                    className="border-b border-l border-border px-3 py-2.5 text-center text-xs text-[#7A9490]"
                  >
    ...
    <div className="space-y-1">
      <p
        className={`font-display text-center text-lg font-semibold tabular-nums ${
          winner ? "text-[#38A047]" : "text-foreground"
        }`}
      >
        {formatScore(value)}
      </p>
      <div className="h-[3px] overflow-hidden rounded-full bg-[#2E3D38]">
        <div
          className={`h-full rounded-full ${winner ? "bg-[#38A047]" : "bg-[#2E3D38]"}`}
          style={{ width: `${pct}%` }}
        />
      </div>
    </div>
