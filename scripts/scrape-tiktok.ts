...
async function runApifyActor(handles: string[]) {
  ...
  const runInfoRes = await fetchWithRetry(`https://api.apify.com/v2/actor-runs/${runId}`, {
    headers: getHeaders(),
  });
  const runInfo = await runInfoRes.json();
  const datasetId = runInfo.data.defaultDatasetId;

  const itemsRes = await fetchWithRetry(`https://api.apify.com/v2/datasets/${datasetId}/items`, {
    headers: getHeaders(),
  });
